...
might be because of no other sounds were present and when we get two sources, the one we opted to hear will be loud 
enough in comparison for separation to work. So we applied the same process to the samples we got from two people 
talking at the same time as to what we did with one with only one person talking.\\

After shifting left microphone we could see some data line up with the right microphone data in figure 
\ref{fig:somelabel}., which seems like it could work and it would separate the unwanted sounds from the samples. But 
when listening to the output, we couldn't hear any audible difference from the original sample. 

\begin{figure}[htp]
  \centering
  \includegraphics[width=0.7\linewidth]{Illustrations/OnePersonOriginalAndSeparated.png}
  \caption{This is not the picture, i will make one sometime which will be shiftedL signal on right}
  \label{fig:somelabel}
\end{figure}

Then we compared output data to original data (see figure \ref{fig:someOtherlabel} and not many visual differences were seen as well

\begin{figure}[htp]
  \centering
  \includegraphics[width=0.7\linewidth]{Illustrations/OnePersonOriginalAndSeparated.png}
  \caption{This is not the picture, i will make one sometime which will be separated signal on right}
  \label{fig:someOtherlabel}
\end{figure}
