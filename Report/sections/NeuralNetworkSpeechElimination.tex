...
The following table \todo{add nr of table} represents the batch size, dropout, learning rate, epochs and samples used for each model.
All the models from the table are further discussed in the next section.

\todo{add table with all things specified}
...
The first model consists of two GRU layers and a fully connected layer as seen in figure todo[add figure nr}.
\todo{add the graph for model 1 here}

In the second model the two GRU layers were replaced by a Bidirectional GRU layer followed again by a fully connected layer as seen in figure todo[add figure nr}.

\todo{add the graph for model 2 here}
...
\todo{add the graph for model 3 here}
